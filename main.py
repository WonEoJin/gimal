...
class Character(Sprite):
  ...
  def hop(self):
    ...
    y_velocity = 1.5  # pixels/(time of iteration)
    x_velocity = 0.6  # pixels/(time of iteration)
    bounce_count = 0
    if self.jump == True:
      while True:
        self.y += y_velocity
        ...
        pen.clear()
        for cws in sprites:  #cws = current working sprite
          cws.render(pen)
        wn.update()
...
while True:
  ...
  if pacman.x >= cherry.x:
    if cherry.image != "x.gif":
      pacman.image = "Angry_pac.gif"
